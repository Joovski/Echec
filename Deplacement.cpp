...
{       int diri[4]={-1,+1,+0,+0};      //direction en ligne
        ...
        {    if(PE->LireCaseEchiquier(idep,jdep)=='t') couleur=1;     //noir
             ...
             {       iarr=idep+diri[d]; //une case plus loin que la case de d�part
                     jarr=jdep+dirj[d];
                     stop=false;
                     while((iarr>=0 && iarr<8 && jarr>=0 && jarr<8)   //dans l'�chiquier
                     ...
                     {  tabPositionArrivee[0][nbPositionArrivee]=iarr;
                        ...
                        nbPositionArrivee++;
                        if(PE->LireCaseEchiquier(iarr,jarr)!=' ') stop=true;  //une pi�ce a �t� rencontr�e
...
{       int sauti[10]={-1,-1,-1,+0,+0,+1,+1,+1,+0,+0};
        ...
        {    if(PE->LireCaseEchiquier(idep,jdep)=='r') couleur=1;
             ...
             {       iarr=idep+sauti[n];
                     jarr=jdep+sautj[n];
                     if(iarr>=0 && iarr<8 && jarr>=0 && jarr<8)//dans l'�chiquier
